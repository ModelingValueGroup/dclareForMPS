...
public class DClareMPS implements TriConsumer<State, State, Boolean>, Universe {
	...
	private static final QualifiedSet<Triple<DObject, DFeature<?>, String>, DMessage> MESSAGE_QSET = QualifiedSet
			.of(m -> Triple.of(m.context(), m.feature(), m.id()));

	protected static final Map<DMessageType, QualifiedSet<Triple<DObject, DFeature<?>, String>, DMessage>> MESSAGE_QSET_MAP = MESSAGE_TYPES
			.sequential().toMap(t -> Entry.of(t, MESSAGE_QSET));
	...
	public final static Observed<DClareMPS, Set<SLanguage>> ALL_LANGUAGES = NonCheckingObserved.of("ALL_LANGAUGES",
			Set.of());
	...
	private final static Setable<DClareMPS, DRepository> REPOSITORY_CONTAINER = Setable.of("REPOSITORY_CONTAINER", null,
			true);
	...
	protected DClareMPS(DclareForMPSEngine engine, ProjectBase project, State prevState, int maxTotalNrOfChanges,
			int maxNrOfChanges, int maxNrOfObserved, int maxNrOfObservers, StartStopHandler startStopHandler) {
		...
		universeTransaction = new UniverseTransaction(this, thePool, prevState, 100, maxTotalNrOfChanges,
				maxNrOfChanges, maxNrOfObserved, maxNrOfObservers, 4, null) {
			...
			@Override
			protected void checkOrphanState(
					Entry<Object, Pair<DefaultMap<Setable, Object>, DefaultMap<Setable, Object>>> e0) {
			}
	...
	private void addTooManyChangesExceptionMessage(DObject context, DFeature feature, TooManyChangesException tmce) {
		DMessage message = new DMessage(context, feature, DMessageType.error, "TOO_MANY_CHANGES",
				"Too many changes, running " + feature + " changes=" + tmce.getNrOfChanges());
		tmce.getLast().trace(message, (m, r) -> {
			m.addSubMessage(new DMessage((DObject) r.mutable(), ((DRule.DObserver) r.observer()).rule(),
					DMessageType.error, " ", //
			...
			m.subMessages().last().addSubMessage(
					new DMessage((DObject) s.mutable(), (DObserved) s.observed(), DMessageType.error, " ", //
	...
	private void addTooManyObservedExceptionMessage(DObject context, DFeature feature, TooManyObservedException tmse) {
		DMessage message = new DMessage(context, feature, DMessageType.error, "TOO_MANY_OBSERVED",
				tmse.getSimpleMessage());
			...
			if (e.getKey() instanceof DObserved) {
				...
				for (Mutable o : e.getValue()) {
					if (o.resolve(context) instanceof DObject) {
						message.addSubMessage(new DMessage((DObject) o.resolve(context), observed, DMessageType.error,
								" ", "Observed: " + observed));
					}
				}
			}
	...
	@SuppressWarnings("rawtypes")
	private void addTooManyObserversExceptionMessage(DObject context, DFeature feature,
			TooManyObserversException tmse) {
		DMessage message = new DMessage(context, feature, DMessageType.error, "TOO_MANY_OBSERVERS",
				tmse.getSimpleMessage());
		for (Entry<Observer, Set<Mutable>> e : tmse.getObservers()) {
			if (e.getKey() instanceof DRule.DObserver) {
				...
				for (Mutable o : e.getValue()) {
					if (o.resolve(context) instanceof DObject) {
						message.addSubMessage(new DMessage((DObject) o.resolve(context), rule, DMessageType.error, " ",
								"Rule: " + rule));
					}
				}
			}
	...
	public void accept(State pre, State post, Boolean last) {
		if (imperativeTransaction != null && !universeTransaction.isKilled()) {
			...
			COMMITTING.set(true);
			try {
				pre.diff(post, o -> o instanceof DObject && !((DObject) o).isDclareOnly(),
						p -> p instanceof DObserved && !((DObserved) p).isDclareOnly()).forEach(e0 -> {
	...
	private class NodeChecker extends IChecker.AbstractNodeChecker<NodeReportItem> {
		...
		@Override
		public void check(SNode object, SRepository repository, Consumer<? super NodeReportItem> consumer,
