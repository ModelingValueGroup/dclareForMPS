...
public class DNode extends DNewableObject<DNode, SNodeReference, SNode> implements SNode {
    ...
    public static Observer<DNode> copyObserver(IAspect aspect, DObserved<DNode, ?> observed, TriConsumer<DNode, DNode, DCopy> action) {
        ...
        return DCopyObserver.of(aspect, observed, node -> {
            Construction construction = node.dAllDerivations().get(direction);
            if (construction != null) {
                Reason reason = construction.reason();
    ...
    protected void activate() {
        if (!isExternal() && isAction() && isRead()) {
            LeafTransaction tx = LeafTransaction.getCurrent();
            if (!isActive(tx.current())) {
                SNode root = dClareMPS(tx).read(() -> tryOriginal().getContainingRoot());
                if (root != null) {
                    SModel model = dClareMPS(tx).read(root::getModel);
                    if (model != null) {
